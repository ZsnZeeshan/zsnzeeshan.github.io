---
layout: home
navTag: Skills
order: 1
title: Skills
description: Showcasing practical expertise in embedded systems, IoT, automation, cloud tools, and software engineering, emphasizing hands‑on development, problem‑solving, and modern technical capabilities.
image:
  path: "/assets/images/icon2.webp"
  alt: "Personal Logo of Muhammad Zeeshan Arshad"
last_modified_at: 2026-07-04
---

## Technical Skills & Tools

**Software Development:** Assembly, Bash, C, C++, Linux System Programming, Python, Ruby, VHDL  
**Hardware & Test Engineering:** Custom SoCs, Oscilloscopes, Semiconductor Testers (V93000), Spectrum/Logic Analyzers, Microcontrollers, SDRs  
**Communication Protocols:** BLE, CAN, I2C, JTAG, MDIO, MQTT, RS-232, RS-485, SPI, SWD  
**Design & Debug Tools:** Autodesk Fusion, Coverage, GDB, KiCAD, MISRA/Coverity Compliance, OpenOCD, Sigrok  
**DevOps & CI/CD:** Automated Test Frameworks, Bazel, CMake, Docker, Git, Jenkins, Make  
**Cloud, IIoT & Data Platforms:** AWS, InfluxDB, IoTStack, Prometheus, Relational & Time-series Databases, SQLite  
**Networking & Security:** Firewalls, Intrusion Detection/Prevention, OpenWRT, SSL/TLS, TCP/IP, UDP, Wireshark, Wi-Fi  
**Monitoring & Reporting:** Grafana, LaTeX, Markdown, Matplotlib, Node-RED, PlantUML, Technical Documentation

## Certifications

{% include pdf_expand.html 
   summary="Atlassian Agile Project Management"
   src="/assets/data/certifications/Atlassian_AgileProjectManagementProfessionalCertificate.pdf" 
%}

{% include pdf_expand.html 
   summary="Docker Foundations"
   src="/assets/data/certifications/Docker_DockerFoundationsProfessionalCertificate.pdf" 
%}

{% include pdf_expand.html 
   summary="Github Copilot Career Essentials"
   src="/assets/data/certifications/Github_CareerEssentialsinGitHubCopilotProfessionalCertificate.pdf" 
%}

{% include pdf_expand.html 
   summary="Microsoft Cybersecurity Career Essentials"
   src="/assets/data/certifications/Microsoft_CareerEssentialsinCybersecurity.pdf" 
%}

{% include pdf_expand.html 
   summary="Microsoft Data Analysis Career Essentials"
   src="/assets/data/certifications/Microsoft_CareerEssentialsinDataAnalysis.pdf" 
%}

{% include pdf_expand.html 
   summary="PagerDuty DevOps"
   src="/assets/data/certifications/PagerDuty_DevOpsProfessionalCertificate.pdf" 
%}

{% include pdf_expand.html 
   summary="TestMu AI Software Testing"
   src="/assets/data/certifications/TestMuAI_SoftwareTestingProfessionalCertificate.pdf" 
%}
